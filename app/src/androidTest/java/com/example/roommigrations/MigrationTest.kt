...
class UserMigrationTest {
    ...
    val helper = MigrationTestHelper(
        ...
        listOf(UserDatabase.MigrationFrom2To3()),
        FrameworkSQLiteOpenHelperFactory()
    )

    @Test
    fun migrationFrom1To2_containsCorrectData() {
        var database = helper.createDatabase(DB_NAME, 1).apply {
            execSQL("INSERT INTO user VALUES('user1@example.com', 'user1')")
        }

        database = helper.runMigrationsAndValidate(DB_NAME, 2, true)

        database.query("SELECT * FROM user").apply {
